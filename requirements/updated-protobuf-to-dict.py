...
TYPE_CALLABLE_MAP = {
    ...
    FieldDescriptor.TYPE_BYTES: lambda b: b.encode("base64").decode(),
...
def get_bytes(value):
    return value.encode('base64').decode()
...
def dict_to_protobuf(pb_klass_or_instance, values, type_callable_map=REVERSE_TYPE_CALLABLE_MAP, strict=True):
    ...
    :param dict values: a dictionary of values. Repeated and nested values are fully supported.
    :param dict type_callable_map: a mapping of protobuf types to callables for setting values on the target instance.
...
def _dict_to_protobuf(pb, value, type_callable_map, strict):
    ...
    for field, input_value, pb_value in fields:
        ...
        if field.label == FieldDescriptor.LABEL_OPTIONAL or input_value is not None:
